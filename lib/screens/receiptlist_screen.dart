...
class _ReceiptListScreenState extends State<ReceiptListScreen> {
  ...
  TextEditingController _receiptnameTextController = TextEditingController();
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                  onPressed: () {
                  FirebaseFirestore.instance.collection("Receipt").doc(docId).delete()
                  .then((value) {
                          _receiptnameTextController.text = '';
                          _amountTextController.text = '';
                          _dateTextController.text = '';
  ...
  body: Column(
    children: [
      SizedBox(height: 10.0), // Add the desired gap height here
      SizedBox(
                          ...
                          child: Row(
                            children: [
                              ...
                              IconButton(
                                onPressed: () {
                                  _receiptnameTextController.text =
                                      snapshot.data!.docs[index]['Receipt name'];
                                  _amountTextController.text =
      ...
      SizedBox(child:SizedBox(height: 55,
  ...
  floatingActionButton: FloatingActionButton.extended(
    ...
    onPressed: () {
      Navigator.push(
          context, MaterialPageRoute(builder: (context) => AddReceiptScreen()));
    },
  ),
);
  ...
  showModalBottomSheet(
    ...
    builder: (BuildContext context) => Container(
      ...
      child: Column(
        children: <Widget>[
          Text("Receipt Detail", style: TextStyle(fontSize: 28, fontFamily: 'Oswald')),
          const SizedBox(height: 10),
          Row(
            children: [
              const SizedBox(width: 10),
              Text("Receipt Name: ", style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
              Text(_receiptnameTextController.text, style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
            ],
          ),
          const SizedBox(height: 10),
          Row(
            children: [
              const SizedBox(width: 10),
              Text("Receipt Amount: RM", style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
              Text(_amountTextController.text, style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
            ],
          ),
          const SizedBox(height: 10),
          Row(
            children: [
              const SizedBox(width: 10),
              Text("Receipt Date: ", style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
              Text(_dateTextController.text, style: TextStyle(fontSize: 18, fontFamily: 'Oswald')),
            ],
          ),
          const SizedBox(height: 10),
          Container(
...
void _downloadImage(String imageUrl) async {
  ...
  if (response.statusCode == 200) {
    ...
    File file = File('${documentDirectory!.path}/$fileName' as String);
    await file.writeAsBytes(bytes);
    final snackBar = SnackBar(content: Text('Image downloaded'));
    ScaffoldMessenger.of(context).showSnackBar(snackBar);
  } else {
    final snackBar = SnackBar(content: Text('Failed to download image'));
    ScaffoldMessenger.of(context).showSnackBar(snackBar);
  }
}
